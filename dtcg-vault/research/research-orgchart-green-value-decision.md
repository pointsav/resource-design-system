# DESIGN-RESEARCH — Client A chart green value drift vs design-system `--wf-green`
...
| Client A WCP_JW3, SPV2_Detailed_JW2 (hardcoded) | `#198038` | IBM Carbon Green 70 — saturated |
| Client A charts (tint, hardcoded) | `#DEFBE6` | IBM Carbon Green 10 |

The two Client A charts that use green (WCP_JW3 for WCP / operational entity
...
two Client A charts (Organization_JW2, SPV2_JW2) do not use the green role,
...
hardcoded `#198038` (as the Client A charts do via inline `.token-green`
...
"Accounting / finance / operations domain accent." The Client A WCP chart uses
...
~~Adopt `--wf-green: #54924E` as the canonical green. The Client A charts
...
and the Client A charts were already authored with it and verified by Creative Designer.
...
- `project-orgcharts` Client A WCP charts: hardcoded `#54924E` reverted to `#198038` (charts already correct; an intermediate commit that changed them is nullified)
...
- `inputs/current-org-chart-html/INVESTOR_RELATIONS_2026-05-27_Chart_Client A_WCP_JW3.html`
- `inputs/current-org-chart-html/INVESTOR_RELATIONS_2026-05-27_Chart_Client A_SPV2_Detailed_JW2.html`
...
to match the token. Client A WCP_JW2 and WCP_JW3 updated in project-orgcharts
